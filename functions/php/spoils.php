...
function levelUp($poke){
	...
	if($move_list[$poke[level]] != ''){
		$replyData[] = ['type' => 'learnMove', 'level' => $poke[level], 'poke' => $poke[uuid]];
	}
...
function calcExp($aPoke, $dPoke, $num, $draw){
	global $replyData;

	$exp_vars[a] = 1;					// a = 1 if wild : 1.5 if trainer's poke
	$exp_vars[b] = $dPoke[base_exp];	// b = base yeild of pokemon
	$exp_vars[e] = 1;					// e = (lucky egg) ? 1.5 : 1
	$exp_vars[f] = 1;					// f = (affection >= 2 hearts) ? 1.2 : 1
	$exp_vars[L] = $dPoke[level];		// L = level of fainted poke
	$exp_vars[Lp] = $aPoke[level];		// Lp = Level of victorious pokemon
	...
	$exp_per = round($exp_gain / count($num));

	$ii = 0;
	$exp_text;
	if($draw){
		$pct = ($aPoke[exp] + $exp_per) / $aPoke[exp_to_lvl];
		$pct = ($pct > 1) ? 1 : $pct;
		$replyData[] = ['type' => 'drawEXP', 'bar' => round($pct * 100), 'uuid' => $aPoke[uuid], 'level' => $aPoke[level]];
	}
...
function trainEV($poke, $dPoke){
	$ev_gains = explode(',', $dPoke[ev]);
	foreach($ev_gains as $k => $v){
		...
		if($v > 0){
			...
			if($poke[evs][$k] > 252){
				$return = 252 - $poke[evs][$k];
				$poke[ev_totals] += $return;
				$poke[evs][$k] = 252;
			}
			elseif($poke[ev_totals] > 510){
				$return = 510 - $poke[ev_totals];
				$poke[evs][$k] += $return;
				$poke[ev_totals] = 510;
			}
		}
...
function spoils($aPoke, $dPoke){
	...
	/***	Non-Wild Encounter
		$victory = 1;
		victory ? {
			wild_encounter ? roll for drops : '';
			trainer_encounter ? give prize money : '';
			custom_encounter ? encounter flag/give stuff : '';
		}
	***/
	$replyData[] = ['type' => 'text', 'text' => 'Wild '.$dPoke['nickname'].' has fainted.'];
	$replyData[] = ['type' => 'victory'];

	if($aPoke[ev_totals] < 510)
		$pokes[trainer][$_POST[poke]] = trainEV($aPoke, $dPoke);
	...
	foreach ($battleLog[participated] as $key => $value) {
		if($victory)
			unset($pokes[trainer][$key][modifiers]);
	
		$pokes[trainer][$key] = calcExp($pokes[trainer][$key], $dPoke, count($battleLog[participated]), ($pokes[trainer][$key][uuid] == $aPoke[uuid]) ? true : false);
	}
